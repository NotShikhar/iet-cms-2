    ...
    <section className="py-20 lg:py-28">
      <div className="container-x">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <a href="https://times.ietdavv.edu.in/" target="_blank" rel="noreferrer" className="btn btn-secondary shrink-0">Read IET Times <ArrowUpRight className="h-4 w-4" /></a>
        </div>

        <div className="mt-12 grid gap-6 lg:grid-cols-2">
          <Reveal>
            <a href={lead.url} target="_blank" rel="noreferrer" className="group relative block h-full min-h-[420px] overflow-hidden rounded-3xl bg-navy-900">
              ...
              <div className="absolute bottom-0 p-7 text-white">
                ...
                <h3 className="font-display mt-3 text-2xl font-bold leading-snug">{lead.title}</h3>
          ...
          <Stagger className="flex flex-col gap-4">
            {rest.slice(0, 4).map((n) => (
              <StaggerItem key={n.id}>
                <a href={n.url} target="_blank" rel="noreferrer" className="card card-hover group flex gap-4 p-4">
                  {n.image ? <img src={n.image} alt="" loading="lazy" className="h-24 w-28 shrink-0 rounded-xl object-cover sm:h-28 sm:w-40" /> : <div className="h-24 w-28 shrink-0 rounded-xl bg-navy-50 sm:h-28 sm:w-40" />}
                  <div className="min-w-0">
                    ...
                    <h3 className="font-display mt-1.5 line-clamp-2 text-base font-bold text-ink group-hover:text-navy-700">{n.title}</h3>
                    <p className="mt-1.5 line-clamp-2 text-sm text-slate-600">{n.excerpt}</p>
                  </div>
                </a>
              </StaggerItem>
            ))}
        ...
        <Stagger className="mt-16 grid gap-5 md:grid-cols-3">
          {[
            ...
            <StaggerItem key={t}>
              <Link to={to} className="card card-hover block h-full p-6">
                <span className="grid h-11 w-11 place-items-center rounded-xl bg-navy-50 text-navy-600"><Icon className="h-5 w-5" /></span>
                <h3 className="font-display mt-4 text-lg font-bold text-ink">{t}</h3>
        ...
        <Reveal className="mt-16">
          <div className="relative overflow-hidden rounded-3xl border border-line bg-gradient-to-br from-white to-navy-50 p-8 sm:p-12">
            <div className="grid-bg absolute inset-0 opacity-60" />
            <div className="relative grid items-center gap-8 lg:grid-cols-2">
              <div>
                <span className="eyebrow">ACIIE · Innovation & Incubation</span>
                <h3 className="font-display mt-3 text-3xl font-bold text-ink">Atal Centre for Innovation, Incubation & Entrepreneurship</h3>
                <p className="mt-3 text-slate-600">ACIIE hosts the Institute’s Innovation Council and the student-run E-Cell, and invites innovative project proposals from students for incubation support.</p>
                <Link to="/page/aciie" className="btn btn-primary mt-6">About ACIIE <ArrowRight className="h-4 w-4" /></Link>
              </div>
          ...
          <Reveal className="mt-16">
            <div className="flex items-end justify-between">
              <div>
                ...
                <h3 className="font-display mt-2 text-2xl font-bold text-ink">From the institute's YouTube channel</h3>
              </div>
              <a href={content.settings.social.youtube} target="_blank" rel="noreferrer" className="text-sm font-semibold text-navy-600 hover:underline">All videos</a>
            </div>
